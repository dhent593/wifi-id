...
export const isPlaceholder = !process.env.NEXT_PUBLIC_SUPABASE_URL || 
                             ...
                             process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY.includes('your-actual-anon-key-here');

// Helper to determine if we are running in local fallback mode
const getUseLocalMode = () => {
    if (typeof window === 'undefined') return isPlaceholder;
    if (isPlaceholder) return true;
    return localStorage.getItem('wifi_use_local_mode') === 'true';
};

const setUseLocalMode = (val) => {
    if (typeof window !== 'undefined') {
        localStorage.setItem('wifi_use_local_mode', val ? 'true' : 'false');
    }
};
...
export const db = {
    ...
    async signIn(email, password) {
        ...
        try {
            ...
            if (res.error) {
                if (isLocalCheck) {
                    // Fallback to local session since cloud failed
                    const session = { user: { email }, timestamp: Date.now() };
                    localStorage.setItem('wifi_mock_session', JSON.stringify(session));
                    setUseLocalMode(true);
                    return { data: session, error: null };
    ...
    async insertPelanggan(userData) {
        // Generate a clean 6-character short token (e.g. BUDI9A)
        const nameClean = userData.name.replace(/[^a-zA-Z]/g, '').substring(0, 4).toUpperCase();
        const randChars = Math.random().toString(36).substring(2, 4).toUpperCase();
        const token = `${nameClean}${randChars}`;
        ...
        return await supabase.from('pelanggan').insert([payload]);
    },
